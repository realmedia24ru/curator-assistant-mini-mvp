...
SYSTEM_PROMPT = (
    ...
    "Сгенерируй РОВНО 3 варианта: 1) Коротко по делу; 2) Эмпатия + шаги; 3) Ссылочный. "
    "Для ссылочного варианта ИСПОЛЬЗУЙ СТРОГО эти плейсхолдеры (без склонений и изменений): "
    "<правила> <анкета> <вводная> <база_защит>. "
    "Выведи JSON-список строк: [\"...\",\"...\",\"...\"]."
)

COURSE_HINTS = (
    "Правила чата: <правила>; Анкета: <анкета>; Вводная лекция: <вводная>; "
    "Платформа: onstudy.org; База защит: <база_защит>."
)

# Автозамена плейсхолдеров на реальные ссылки (устойчиво к склонениям)
LINKS = {
...
def expand_links(txt: str) -> str:
    """
    Подставляет ссылки не только для точных плейсхолдеров, но и для частых вариантов
    со склонениями: <анкету>, <вводную>, <правил>, <база защит> и т.п.
    """
    patterns = [
        (r"<\s*правил[а-я]*\s*>", LINKS["<правила>"]),
        (r"<\s*анкета[а-я]*\s*>", LINKS["<анкета>"]),
...
# --- КЛЮЧЕВАЯ ДОБАВКА: класс "подтверждение/благодарность" (ack) ---
ACK_WORDS = [
    "спасибо", "спасиб", "благодар", "ок", "окей", "ага", "угу",
    "понял", "поняла", "ясно", "принял", "приняла", "супер", "класс",
    "огонь", "отлично", "ok", "thx", "thanks"
]
...
def is_ack(text: str) -> bool:
    ...
    if not t or "?" in t:
        return False
    # короткие/подтверждающие реплики: до 6 слов или полностью из эмодзи/междометий
    words = re.findall(r"\w+", t, flags=re.UNICODE)
    if len(words) <= 6 and any(w in t for w in ACK_WORDS):
        return True
    # голые эмодзи/реакции без слов
...
def rule_suggestions(user_text: str) -> List[str]:
    ...
    # Подсказка для быстрой сборки ответа
    def S(a, b):
        return [a, b, ""]  # 3-й вариант оставляем пустым для LLM

    # 0) Подтверждение/благодарность — ДОЛЖНО идти первым
    if is_ack(t):
        return S(
            "Пожалуйста! Если появятся вопросы по материалам — пишите сюда, поможем.",
    ...
    # анкета
    if any(k in t for k in ["анкета", "анкету", "анкеты", "форма", "заполнить анкет"]):
        return S(
    ...
    if any(k in t for k in ["встреч", "знакомств", "расписан", "график", "зум"]):
        return S(
            "Ближайшие встречи — в закрепе/анонсе чата.",
            "Проверьте закреплённые сообщения и анонсы в чате — там актуальное расписание."
        )
    ...
    if any(k in t for k in ["аттест", "свидетельств", "документ об образован"]):
        return S(
            ...
            "Если что-то не добрали, подскажу, как закрыть быстро."
        )
...
if any(k in t for k in ["не работает", "не загружается", "платформа", "сайт не открывается", "ошибка входа", "проблема с доступом"]):
        return S(
            "💻 Попробуйте перезагрузить страницу или очистить кеш браузера. Также можно попробовать другой браузер.",
            "💻 Рекомендую проверить подключение к интернету и попробовать зайти с другого устройства. Если проблема сохраняется, напишите в поддержку платформы."
        )
    # по умолчанию
    return S(
  ...
  # fallback — если нет совпадений, вернём пустые варианты, чтобы все три сгенерировал ИИ
    return ["", "", ""]
